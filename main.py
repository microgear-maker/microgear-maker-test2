    ...
    try:
        ...
        for item in data.get("result", {}).get("list", []):
            ...
            if size > 0:
                ...
                print(f"Загружена позиция: {symbol} {side} {size} @ {avg_price}")
...
def send_photo(img):
    ...
    requests.post(url, data={"chat_id": TG_CHAT_ID}, files={"photo": ("card.png", buf, "image/png")})
    ...
    try:
        if existing and existing["side"] == side:
            ...
            if remaining <= 0:
                ...
                img = make_card("close", symbol, side, price_fmt, total_qty)
            else:
                open_positions[symbol]["qty"] = remaining
                img = make_card("partial", symbol, side, price_fmt, total_qty)
            send_photo(img)
        ...
        print(f"Отправлено: {symbol} {side} {total_qty}            "https://github.com/googlefonts/roboto/raw/main/src/hinted/Roboto-Regular.ttf",
            FONT_REG
        )
        urllib.request.urlretrieve(
            "https://github.com/googlefonts/roboto/raw/main/src/hinted/Roboto-Bold.ttf",
